...
s = [0] * 200
m = [0] * 200
print('compute the mean values and standard deviation')
...
print('compute the p(Target=1|X=xi)')
...
for r in train.index:
    ...
    if ct % 25000 == 0:
        print('train', r, 'has target =', train.iloc[r, 1], 'and prediction =', p)
    pred[ct] = p
    ct += 1
print('###############')
...
sub['target'] = pred
sub.to_csv('./modified_naive_bayes_%s.csv' % dt, index=False)
print('###############')
