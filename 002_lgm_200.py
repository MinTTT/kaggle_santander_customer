...
print('Add counts features to train sets')
train_c = add_feature(train, ori_feature)
print('Add counts features to test sets')
test_c = add_feature(test, ori_feature)
...
for j, fea_na in enumerate(ori_feature):
    for i, (trn_idx, val_idx) in enumerate(
        ...
        model = lgb.train(param, trn_data, 126,
...
out1.to_csv(r'./002_lgm_200_train_%s.csv' % dt, index=False, header=False)
out2.to_csv(r'./002_lgm_200_oof_%s.csv' % dt, index=False, header=False)
...
output.to_csv(r'./002_lgm_200_log_%s.csv' % dt, index=False)
print('Finished !')
